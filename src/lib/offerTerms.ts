// Standardvillkor som förifylls i nya offerter. Texten kan redigeras per offert.
// Utkast – bör granskas av jurist innan skarpt bruk.
export const DEFAULT_OFFER_TERMS = `Allmänna villkor – SmartKlimat N3prenad AB

Utkast – ska granskas av jurist innan det används skarpt. Detta är ett förslag baserat på vanliga svenska villkor för konsument- och entreprenadtjänster, anpassat från ett referensavtal. Det är inte juridisk rådgivning.

Säljare: SmartKlimat N3prenad AB, org.nr 559026-6630, Morsstigen 3, 141 71 Segeltorp. VAT: SE559026663001. Tel: 070-719 72 35. E-post: n3prenad@smartklimat.org. Webb: www.n3prenad.se.

Dessa villkor gäller för avtal mellan SmartKlimat N3prenad AB ("N3prenad") och kunden om utförande av bygg-, renoverings- och installationsarbeten (t.ex. tak, badrum, fasad, fönster och dörrar) samt leverans av tillhörande material.

Konsument eller företag. Är kunden konsument (privatperson) tillämpas konsumenttjänstlagen (1985:716) på arbetet och konsumentköplagen (2022:260) på material som N3prenad levererar, i tillämpliga delar. Är kunden företag eller bostadsrättsförening gäller i första hand avtalet och dessa villkor, med köplagen (1990:931) som utfyllande reglering; konsumentskyddande regler och ROT-avdrag tillämpas då inte.

1. Offert och giltighet
Offert från N3prenad är preliminär och bindande endast om den bygger på riktiga och fullständiga uppgifter från kunden. Offerten gäller i 30 dagar från utställandet om inte annat anges. N3prenad kan göra sedvanlig kreditkontroll innan avtal ingås.

2. Avtal
Avtal uppkommer när kunden accepterar offerten – genom elektronisk signering/accept via N3prenads system, eller genom skriftlig bekräftelse via e-post eller sms. Bekräftelse per e-post eller sms likställs med skriftlig överenskommelse. Avviker kundens accept från offerten genom tillägg eller ändring ses den som en ny förfrågan som N3prenad kan anta eller avböja.

3. Pris
Priset framgår av offerten och är fast om inte annat anges. Arbete eller material som inte uttryckligen ingår i offerten omfattas inte av priset. Priset bygger på kundens uppgifter; kunden ansvarar för merkostnader till följd av felaktiga eller ofullständiga uppgifter.

4. ÄTA-arbeten (ändringar och tillägg)
N3prenad har rätt till ersättning för ändrings- och tilläggsarbeten. Sådana överenskoms särskilt, så snart behovet upptäcks. Skriftlighet eftersträvas; bekräftelse via sms eller e-post räknas som skriftlig. Utan särskild prisöverenskommelse utförs ÄTA på löpande räkning enligt N3prenads vid var tid gällande prislista.

5. ROT-avdrag (endast konsument)
För arbete som berättigar till ROT-avdrag dras preliminärt skattereduktion direkt på fakturan om kunden lämnat de uppgifter som krävs (personnummer och fastighetsbeteckning) samt har avdragsutrymme. Får N3prenad inte ut beloppet från Skatteverket är kunden skyldig att betala beloppet i efterhand. Endast arbetskostnad är ROT-berättigad; material och resor omfattas inte.

6. Betalning
Faktura skickas efter utfört arbete eller enligt särskild betalningsplan. Betalningsvillkor framgår av fakturan. Vid försenad betalning utgår dröjsmålsränta enligt räntelagen samt lagstadgade påminnelse- och inkassoavgifter.

7. Tider och förseningar
Tider i offert/avtal är preliminära om inte annat anges. N3prenad ska utan dröjsmål informera kunden om förseningar och orsak. Vid väsentlig försening har kunden de rättigheter som följer av tillämplig lag.

8. Kundens medverkan
Kunden ska i god tid lämna riktiga uppgifter, ge fritt tillträde till arbetsplatsen samt tillhandahålla el, vatten och uppställningsyta utan kostnad. Kunden ansvarar för att flytta lös egendom och skydda ömtåliga ytor om inte annat avtalats.

9. Underentreprenörer
N3prenad har rätt att anlita underentreprenörer för hela eller delar av arbetet och ansvarar för dessa som för eget arbete.

10. Material
Material som ingår förblir N3prenads egendom till dess full betalning erlagts (återtagandeförbehåll), i den utsträckning det är möjligt enligt lag.

11. Avbeställning (konsument)
Konsument har rätt att avbeställa arbete som inte utförts mot ersättning till N3prenad enligt konsumenttjänstlagen, inklusive ersättning för redan utfört arbete, kostnader för material som inte kan returneras samt skälig ersättning för förlust.

12. Garanti och reklamation
Reklamation ska ske inom skälig tid efter att felet upptäckts eller borde ha upptäckts. För konsumenttjänster gäller reklamationsfristerna i konsumenttjänstlagen. N3prenad lämnar 3 års garanti på utfört arbete. Avhjälpande sker i första hand genom rättelse. Reklamerar kunden fel som N3prenad inte ansvarar för, står kunden för N3prenads kostnader för felsökning och utredning.

13. Ansvar och force majeure
N3prenad ansvarar inte för indirekta skador, driftavbrott, utebliven vinst eller liknande. N3prenad är befriat från ansvar vid omständigheter utanför dess kontroll, såsom brand, krig, pandemi, myndighetsbeslut, strejk, naturkatastrof eller större avbrott i tekniska system, som N3prenad inte rimligen kunnat undvika eller förutse.

14. Ångerrätt (konsument, avtal utanför affärslokal)
Ingås avtalet hemma hos kunden eller på annan plats utanför N3prenads affärslokal har kunden rätt att ångra köpet inom 14 dagar enligt lagen (2005:59) om distansavtal och avtal utanför affärslokaler. Kunden kan begära att arbetet påbörjas innan ångerfristen löpt ut; utnyttjar kunden därefter ångerrätten ska kunden ersätta N3prenad för redan utfört arbete och nedlagda kostnader i proportion till avtalet. Ångermeddelande lämnas skriftligen till n3prenad@smartklimat.org. Standardformulär finns hos Konsumentverket (konsumentverket.se).

15. Personuppgifter (GDPR)
N3prenad behandlar kundens personuppgifter för att fullgöra avtalet och uppfylla rättsliga skyldigheter, i enlighet med gällande dataskyddslagstiftning (GDPR). Kunden har rätt till information om behandlingen och kan kontakta N3prenad för rättelse eller frågor.

16. Försäkring
N3prenad innehar ansvarsförsäkring för sin verksamhet. Kunden ansvarar för att ha gällande försäkring för förvarings- och arbetsplatsen.

17. Ändringar och meddelanden
Ändringar och tillägg till avtalet ska ske skriftligen för att gälla; bekräftelse via e-post eller sms räknas som skriftlig. Delar kunden hushåll med annan myndig person får den personen ingå bindande överenskommelse för kundens räkning, om inte kunden angett annat innan avtalet ingicks.

18. Tvist
Svensk lag tillämpas på avtalet. Tvist prövas av svensk allmän domstol. Konsument kan även vända sig till Allmänna reklamationsnämnden (ARN) eller kommunal konsumentvägledning.`;
